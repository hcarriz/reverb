...
func (t *TodoQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "content":
			if _, ok := fieldSeen[todo.FieldContent]; !ok {
				selectedFields = append(selectedFields, todo.FieldContent)
				fieldSeen[todo.FieldContent] = struct{}{}
			}
		case "due":
			if _, ok := fieldSeen[todo.FieldDue]; !ok {
				selectedFields = append(selectedFields, todo.FieldDue)
				fieldSeen[todo.FieldDue] = struct{}{}
			}
		case "priority":
